<?php 
    ...
    // Figure out current delivery option
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delivery'])) {
        // Use what the user just selected
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delivery'])) {
        ...
        if ($email === '') {
            $deliveryErrors[] = "Email is required";
        }
        ...
        if ($deliveryOption === 'delivery') {
            ...
            if ($zip === '') {
                $deliveryErrors[] = "ZIP code is required for delivery";
            }
        } else {
            // pickup only: we don't require address fields
            $address1 = $_POST['address1'] ?? null;
            $address2 = $_POST['address2'] ?? null;
            $city     = $_POST['city'] ?? null;
            $zip      = $_POST['zipcode'] ?? null;
        }
    ...
    // apply discount to totals again (after possible changes)
    $subTotalAfterDiscount = max(0, $subTotal - $discountAmount);
    $tax   = round($subTotalAfterDiscount * $taxRate, 2);
    $total = $subTotalAfterDiscount + $tax;
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["checkout"])) {
        ...
        if (empty($paymentErrors)) {
            ...
            if ($deliveryType === 'delivery') {
                ...
                $address1 = $address2 = $city = $zipcode = null;
            }
            ...
            try {
                ...
                $stmtOrder->execute([
                    $userId,
                    $_SESSION['checkout_delivery']['email'] ?? null,
                    $deliveryType,
                    $address1,
                    $address2,
                    $city,
        ...
        <div class="checkout-main">
            <div class="checkout-container">
                <form method="post" class="checkout-box delivery">
                    ...
                    <!-- pickup method -->
                    <div class="delivery-div <?= $deliveryOption === 'pickup' ? : 'is-hidden' ?>" id="pickupFields">
                        <p>You selected pickup</p>
                    </div>

                    <!-- delivery method -->
                    <div class="delivery-div <?= $deliveryOption === 'delivery' ? : 'is-hidden' ?></div>" id="deliveryFields">
                        ...
                        <input
                            ...
                            value="<?= htmlspecialchars($zipDb ? $zipDb : ($_SESSION['checkout_delivery']['zipcode'] ?? '')) ?>"
                        />
                    </div>
                ...
                <form method="post" id="payment-box" class="checkout-box payment <?= $deliverySuccess ? '' : 'disabled' ?>">
                    ...
                    <div class="flex-row">
                        ...
                        <div class="flex-column">
                            <label for="cvv-input">CVV/CVV2</label>
                            <input
                                name="cvv-input"
                                id="cvv-input"
                                type="text"
